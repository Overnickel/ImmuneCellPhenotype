...
#         self.pca.fit(data.iloc[:-1, :].values.astype(np.float64).T)
...
class ImmuneDataset(Dataset):
    ...
    def __init__(self, data, target, pca_components):
        scaler = StandardScaler()
        # pca = PCA(n_components=pca_components)
        scaler.fit(data)
        data = scaler.fit_transform(data)
        # pca.fit(data)
        # self.data = pca.transform(data)
        self.data = data
    ...
    def __getitem__(self, i):
        sample = self.data[i, :], self.target[i]
        sample = torch.from_numpy(sample[0]).type(torch.FloatTensor), torch.LongTensor([sample[1]])
        return sample
...
def main():
    ...
    parser.add_argument('--pca-components', type=int, default=50, metavar='N',
                        help='pca components (default:70)')
    ...
    parser.add_argument('--split-ratio', type=float, default=0.8, metavar='SR',
                        help='ratio: trainset / totalset (default: 0.8)')
    ...
    df_input = pd.read_csv(args.input_path, sep='\t', index_col=0).values.T
    df_data, df_target = df_input[:, :-1].astype(np.float64), df_input[:, -1].astype(str)
    df_phenotype = {'desert': 0, 'excluded': 1, 'inflamed': 2}
    df_target = [df_phenotype[t] for t in df_target]
    v_train, v_test, t_train, t_test = train_test_split(df_data, df_target, train_size=args.split_ratio, random_state=args.seed)
    trainset = ImmuneDataset(v_train, t_train, args.pca_components)
    testset = ImmuneDataset(v_test, t_test, args.pca_components)
    ...
    model = Net(args.pca_components).to(device)
